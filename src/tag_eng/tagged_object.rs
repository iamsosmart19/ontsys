...
impl TaggedObject {
    ...
    pub fn from(name: String, tag_store: &TagNameStore, fl: String, tags: &[TagId]) -> Self {
...
mod tests {
    ...
    fn test_from() {
        ...
        let _mfw: TaggedObject = TaggedObject::from("myface", &store, "./myface.png".to_string(), &[]);
    }
    ...
    fn test_add_tag_from_str() {
        let store: TagNameStore =
            TagNameStore::from(&["red", "yellow", "brown", "green", "blue", "black"]);
        let mut mfw: TaggedObject = TaggedObject::from("myface", &store, "./myface.png".to_string(), &[]);
